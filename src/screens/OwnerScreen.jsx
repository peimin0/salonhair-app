...
export default function OwnerScreen({ designers, entries }) {
  ...
  return (
    <div style={{ padding: '20px 16px 100px' }}>
      <div style={{ fontSize: 13, color: '#6E5B68' }}>{currentMonthDisplay()} · 團隊總覽</div>
      ...
      {designers.length === 0 ? (
        <div style={{
          ...
          textAlign: 'center', color: '#6E5B68', fontSize: 13, lineHeight: 1.7,
        }}>
        ...
        <>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
            <div style={{ flex: 1, background: '#FFFFFF', borderRadius: 16, padding: '14px 16px', border: '1px solid #EBE2E6' }}>
              <div style={{ fontSize: 11, color: '#6E5B68', fontWeight: 600 }}>團隊總業績</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#2B1E2A', marginTop: 4 }}>${fmt(totalRevenue)}</div>
            </div>
            <div style={{ flex: 1, background: '#FFFFFF', borderRadius: 16, padding: '14px 16px', border: '1px solid #EBE2E6' }}>
              <div style={{ fontSize: 11, color: '#6E5B68', fontWeight: 600 }}>在職設計師</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#2B1E2A', marginTop: 4 }}>{roster.length}</div>
            </div>
            <div style={{ flex: 1, background: '#FFFFFF', borderRadius: 16, padding: '14px 16px', border: '1px solid #EBE2E6' }}>
              <div style={{ fontSize: 11, color: '#6E5B68', fontWeight: 600 }}>高風險人數</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#B8756B', marginTop: 4 }}>{highRisk.length}</div>
            </div>
          </div>
